...
export default function CategoryPage() {
  ...
  // 1. URL se category ka naam nikalna (e.g. suits, sarees)
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        setLoading(true);
        // 2. Dynamic Category ke hisab se backend se data mangana
  ...
  return (
    <div className="bg-white min-h-screen pb-20 font-sans">
      ...
      <div className="bg-gray-50 py-12 text-center">
        <h1 className="text-4xl md:text-5xl font-serif font-bold text-gray-900 mb-2 capitalize">
          {categoryname}
        </h1>
        ...
        <div className="flex-1">
          <div className="flex justify-between items-center mb-8 border-b pb-4">
            <p className="text-gray-500 font-medium tracking-tight">
              Showing {products.length} Designs in {categoryname}
            </p>
          </div>

          {products.length === 0 ? (
            <div className="text-center py-20 text-gray-400 font-serif text-xl">
              Is category mein abhi koi products nahi hain.
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12">
              {products.map((product) => (
                <div key={product.id} className="group flex flex-col">
                  ...
                  {/* Image Container with Safe Detail Link */}
                  <div className="relative aspect-[3/4] overflow-hidden rounded-2xl bg-gray-50 mb-4 shadow-sm">
                    ...
                    {/* Quick Add - Price handling fixed */}
                      ...
                      <button 
                        onClick={(e) => {
                          ...
                          addItem({
                            id: product.id,
                            name: product.name,
                            price: `₹${product.selling_price}`, // Safe price string
                  ...
                  <Link href={`/product/${product.id}`} className="space-y-1">
                    <h3 className="font-serif text-lg text-gray-900 group-hover:text-primary transition-colors line-clamp-1">
                      {product.name}
                    </h3>
