...
import { useEffect, useState } from "react"; // Hook for API call
import Link from "next/link"; // Required for linking to detail page
import { Filter, ChevronDown, ShoppingBag, Loader2 } from "lucide-react";
import { useCartStore } from "@/store/useCartStore"; // Add to Cart functionality

export default function SuitsPage() {
  const [suits, setSuits] = useState<any[]>([]); // Backend data store
  const [loading, setLoading] = useState(true);
  const addItem = useCartStore((state) => state.addItem);

  // API se data mangane ka logic
  useEffect(() => {
        ...
        <div className="flex-1">
          <div className="flex justify-between items-center mb-6">
            <p className="text-gray-500 text-sm">
              {loading ? "Loading..." : `Showing ${suits.length} Products`}
            </p>
          </div>
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 gap-4">
              ...
              <p className="text-gray-500 font-medium">Fetching Designer Suits...</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {suits.map((product) => (
                <div key={product.id} className="group cursor-pointer">
                  {/* Link added to wrap Image */}
                  <Link href={`/product/${product.id}`}>
                    <div className="relative h-[400px] overflow-hidden rounded-lg bg-gray-100 mb-4 shadow-sm">
                      ...
                      <div className="absolute bottom-4 left-4 right-4 translate-y-full group-hover:translate-y-0 transition-transform duration-300">
                        <button 
                          onClick={(e) => {
                            e.preventDefault(); // Navigation roko cart button ke liye
                            addItem({
                              id: product.id,
                              name: product.name,
